...
bool BumpMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../new_engine/bumpmap.vs", L"../new_engine/bumpmap.ps");
	if (!result)
	{
		return false;
	}
	...
	D3DXVECTOR4 diffuseColor)
{
	...
	deviceContext->PSSetShaderResources(0, 2, textureArray);// Lock the light constant buffer so it can be written to.
